...
bool Duplicates<duplicates>::scan(const wchar_t * sDir)
{
    ...
    if (OnStatus)
    {
        OnStatus(OnStatusContext, L"Scanning...");
    }
			...
			else
			{
                try
                {
                    ...
                    if (can_scan == true)
                    {
                        ...
                        if (OnNewFile)
                        {
                            OnNewFile(OnNewFileContext, current_path, (unsigned long long)size, L"", 0);
                        }
                    }
                    file_count++;
                }
...
void Duplicates<duplicates>::find_the_duplicates()
{
	...
	for (std::set<duplicates>::iterator itr = itr1; itr != itr2; itr++)
	{
        ...
        if (OnNewFile)
        {
            std::wstring strHash(Hash.begin(), Hash.end());
            OnNewFile(OnNewFileContext, itr->c_str(), 0, strHash.c_str(), 1);
        }
		//std::cout << Hash << std::endl;
		if(Hash!="failed")
            check_hash_signatures(Hash, *itr);
	}
}
...
void Duplicates<duplicates>::get_duplicates()
{
	...
	for (std::set<duplicates>::iterator itr = itr1; itr != itr2; itr++)
	{
        ...
        if (OnNewFile)
        {            
            OnNewFile(OnNewFileContext, itr->c_str(), 0, L"", 2);
        }
	}
